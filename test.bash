...
# テストケース1: 低脂肪率 (10%未満)
out=$(echo "8.5 170 65" | ./kadai1_taikei) || ng "$LINENO"
exp="BMI: 22.49\nあなたは痩せ体型と判別されました。\nしっかりご飯を食べましょう。"
[ "$(echo -e "$out")" = "$(echo -e "$exp")" ] || ng "$LINENO"

# テストケース2: 標準脂肪率 (10%～25%)
out=$(echo "20 170 65" | ./kadai1_taikei) || ng "$LINENO"
exp="BMI: 22.49\nあなたは標準体型と判別されました。\nこのままの体型をしっかり維持していきましょう。"
[ "$(echo -e "$out")" = "$(echo -e "$exp")" ] || ng "$LINENO"

# テストケース3: 高脂肪率 (25%以上)
out=$(echo "30 170 65" | ./kadai1_taikei) || ng "$LINENO"
exp="BMI: 22.49\nあなたは肥満体型と判別されました。\nしっかり運動しましょう。"
[ "$(echo -e "$out")" = "$(echo -e "$exp")" ] || ng "$LINENO"

# テストケース4: 不正な入力 (脂肪率が範囲外)
out=$(echo "105 170 65" | ./kadai1_taikei) || ng "$LINENO"
